...
from weighted_binary_loss import WeightedBinaryCrossEntropyLoss
...
# argparser
parser = argparse.ArgumentParser(description='Pneumonia CheXNet Classifier Training')
...
# model
densenet = DenseNet121(out_size=num_classes)
densenet.to(device)
...
if flags.resume:
    ...
    densenet.load_state_dict(checkpoint['net'])
    updating_parameters = densenet.parameters()
elif flags.transfer:
    checkpoint = torch.load(flags.checkpoint)
    updating_parameters = densenet.transfer(state_dict=checkpoint)
else:
    # train from scratch - init weights
    updating_parameters = densenet.parameters()

criterion = WeightedBinaryCrossEntropyLoss(N_POS, N_NEG)
...
def train(epoch):
    ...
    for batch_index, (samples, gts, ws, hs, ids) in enumerate(trainLoader):
        ...
        if torch.cuda.device_count() > 1:
            outputs = nn.parallel.data_parallel(densenet, samples)
        else:
            outputs = densenet(samples)
        ...
        if (batch_index + 1) % snapshot_interval == 0:
            snapshot(epoch, batch_index, densenet, train_loss / (batch_index + 1))
